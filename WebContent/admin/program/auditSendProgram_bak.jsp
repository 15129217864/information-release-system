...
<%
	...
	Users user = (Users) request.getSession().getAttribute("lg_user");//17800147445
	...
	ips0=(String[])ipset.toArray( new  String[ipset.size()]);
	String tempstring="";
    for(int j=0,n=ips0.length;j<n;j++){
	   tempstring+="#"+ips0[j];
	}
	...
	if("1".equals(oppp)){ //审核员审核发送用
	   ...
	   if(null!=batch){
		   String ids="";
			List<ProgramApp> plist= new ProgramAppDAO().getALLProgramAppByStr( conn," where  batch="+batch+" order by program_playdate ,program_playtime ");
           for(int i=0,n=plist.size();i<n;i++){
           		ProgramApp pa= plist.get(i);
        		ids+=pa.getId()+"!";
        		send_ips=pa.getProgram_play_terminal();
	...
	ProgramApp programapp=new ProgramAppDAO().getProgramAppByStr(conn,"where program_id='"+programid+"'");
		   ...
		   terminal=enty.getValue();
			for(int i=0,n=ips.length;i<n;i++){
			   //System.out.println(terminal.getCnt_ip()+"----"+ips[i]);
				if(terminal.getCnt_ip().equals(ips[i])&&terminal.getCnt_islink()!=3){//防止不同MAC，IP一样的情况
				   if(null!=terminal.getCnt_name()){//防止在线的未审核终端的出现
						appiplist.add(terminal);
						if(online_ips.indexOf(terminal.getCnt_ip())==-1)
						   online_ips+="#"+terminal.getCnt_ip();
					}else{
					 //  if(null!=terminal)	
		...
		<script type="text/javascript">
		       ...
		       function checkProgram(clientprojectlist){
			       ...
			       if(clientprojectlist==null || clientprojectlist==""){
				        ...
				        document.getElementById("p_str").innerHTML="正在发送节目，请稍后...";
				        DwrClass.sendProgram('${programid}@${clentips}','${lg_user.name }','${ids}',sendProgramStatus); 
				 ...
				 var cnt_array_num=cnt_array.length;
				  ...
				  function issendok(resultmaps){
						...
						if(allok_num==(cnt_array_num-1)){
							...
							//////////////////*******************激活刷新下载状态，扫描下载状态，并显示。。。。。。。。。
			        		timer=setInterval("DwrClass.clientRequest('${programapp.program_jmurl}@"+sendips+"',clientRequesttatus)", 2000);
				    		...
				    		for(i=0;i<cnt_array_num;i++){
				    			if(document.getElementById(cnt_array[i])){
				    				if(document.getElementById(cnt_array[i]).className=="deff"){//有时发送，出现还没到后台，此处也显示出来
						    			document.getElementById(cnt_array[i]).innerHTML="<span style=color:'red'><img src='/images/error.gif'  height='16px'>发送失败，请检查网络是否连接！</span>";
									...
									<c:if test="${!empty appiplist}">
										...
										<c:forEach items="${appiplist}" var="appip">
											...
											<tr align="center" class="TableTrBg06_" style="height: 25px" onmouseover="this.className='TableTrBg06'" onmouseout="this.className='TableTrBg06_'" >
												...
												<td id="${appip.cnt_ip }" class="deff"  width="34%" >
													等待发送
												</td>
											</tr>
											<tr>
									...
									<c:if test="${!empty appiplist_n}">
										...
										<c:forEach items="${appiplist_n}" var="appip">
											...
											<tr align="center" class="TableTrBg06_" style="height: 25px" onmouseover="this.className='TableTrBg06'" onmouseout="this.className='TableTrBg06_'" >
												...
												<td  id="${appip.cnt_ip }"  width="34%" style="color:maroon"  >
													当前不下载，开机下载
												</td>
											</tr>
											<tr>
