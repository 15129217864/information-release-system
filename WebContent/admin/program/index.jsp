...
<% String left_menu=request.getParameter("left_menu")==null?"":request.getParameter("left_menu");
 ...
 //FirstStartServlet.getUSD();//获取美元指数图片
%>
<html>
	<head>
	...
	<FRAMESET rows="63,30,*" cols="*" border="0">
		<FRAME noresize scrolling="no" frameBorder="0"
			src="/admin/program/ter_header.jsp?title=<%=title %>&left_menu=<%=left_menu %>&zu_id=<%=zu_id %>"
			name="deviceTop" />
		<FRAME noresize scrolling="no" frameBorder="0"
			src="/admin/program/program_list_top.jsp" name="listtop" />
		<FRAME frameBorder="0" scrolling="auto" frameBorder="0"
			src="/rq/programList?left_menu=<%=left_menu %>&zu_id=<%=zu_id %>" 
