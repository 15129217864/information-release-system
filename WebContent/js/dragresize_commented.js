...
function DragResize(myName, config){
	 ...
	 var props = {
			  ...
			  handles: ['tl', 'tm', 'tr','ml',
			            'mr', 'bl', 'bm','br'], // Array of drag handles: top/mid/bot/right.
...
DragResize.prototype.select = function(newElement) {
	...
	with (this){
		 ...
		 if (newElement && (newElement != element) && enabled){
			...
			changeModelBg(element.id.replace("model",""));
			getAttr(element);
			document.getElementById("divid").value=element.id;
...
DragResize.prototype.getModelId=function(){
...
DragResize.prototype.mouseMove = function(e) {
	 ...
	 with (this){
		 ...
		 with (element.style){
			...
			if(elmX<0)elmX=0;
			if(elmY<0)elmY=0;
			if(elmX>maxLeft)elmX=maxLeft;
			if(elmY>maxTop)elmY=maxTop;
			if(elmW>maxLeft)elmW=maxLeft;
			if(elmH>maxTop)elmH=maxTop;
			
			document.getElementById("_left").value=elmX;
		  document.getElementById("_top").value=elmY;
		   document.getElementById("_width").value=elmW;
		    document.getElementById("_height").value=elmH;
		   ...
		   //document.write(left+" "+top+" "+width+" "+height);
		 }
...
DragResize.prototype.resizeHandleSet = function(elm, show) { //����ģ��󴥷�
	with (this){
		 ...
		 for (var h = 0; h < handles.length; h++){
			...
			//document.getElementById('model' + handles[h]).style.backgroundColor = '#ffffff';
		 }
		 //alert("resizeHandleSet");
    }
};
