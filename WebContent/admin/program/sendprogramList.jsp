...
<%
	...
	List list= programdao.getALLProgramAppByStr("where "+str+"  program_app_status=1  order by id desc");
...
showDiv1("查看播放终端",300,300,"/admin/program/showTerminal.jsp?program_id="+program_id);
      ...
      <c:forEach var="programcla" items="${requestScope.programList}">
	   <table width="100%" height="27"  border="0" cellpadding="0" cellspacing="0" >
        <tr  class="TableTrBg06_" onmouseover=this.className="TableTrBg06" onmouseout=this.className="TableTrBg06_" height="35px">
          <td width="10">&nbsp;</td> 
          <td width="15">
          <input type="checkbox" name="checkbox" value="${programcla.id }" >
          </td>             
          <td width="12%"><a href="javascript:;" title="预览节目" onclick="viewProgram('${programcla.program_name }','${programcla.program_jmurl }','${programcla.templateid }',600,450);"><strong>${programcla.program_name }</strong></a></td>
          <td width="12%">${programcla.program_jmurl }</td>
          <td width="12%">${programcla.program_playdate } ${programcla.program_playtime }</td>
          <td width="12%">${programcla.program_enddate } ${programcla.program_endtime }</td>
          <td width="12%">${programcla.program_play_typeZh }</td>  
           <td width="12%">${programcla.program_app_time }</td>
          <td width="12%">${programcla.program_app_statusZh }</td>      
           <td width="12%"><input type="button" class="button" onclick="showterminal('${programcla.id }')" value="查看播放终端"/></td>
