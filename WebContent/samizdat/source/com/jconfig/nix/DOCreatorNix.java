			...
			if ( whichType == kCreateDiskObjectFolder ) {
				if ( newFile.exists() ) {
					...
					if ( newFile.isDirectory() ) {
						return new DiskFileNix( newFile );
					}
					else {
						return null;
					}
...
*/
	...
	static DiskObject createDiskObject( File fl ) {
		...
		try {
			if ( AppUtilsNix.isLinkFile( fl ) ) {
				return new DiskAliasNix( fl );
			}
			else if ( AppUtilsNix.isDrivePath( fl ) ) {
				driveName = AppUtilsNix.pathToDriveName( fl );
				if ( driveName == null )
					return null;
				return new DiskVolumeNix( driveName );
			}
			else {
				return new DiskFileNix( fl );
			}
		}
